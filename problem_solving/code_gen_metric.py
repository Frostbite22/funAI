...
def metric(example,pred,trace=None):
    problem, correct_code, generated_code = example.problem, example.python, pred.generated_code

    correct = f"the generated code : {generated_code.code} \n should be considered correct and of high quality if it has the same complexity of {correct_code} or lower and validates the test cases for the given problem {problem}.\nDoes the generated code meet this criteria? Answer Yes or No."
    print("Problem:",problem,"\n")
    print("Generated code:",generated_code.code,"\n")
    print("Correct code:",correct_code,"\n")

    with dspy.context(lm=llama3):
        correct = dspy.Predict(CodeGenAssessSignature)(assessed_code=generated_code.code,assessement_question=correct)
